...
const ProductCard = ({ product, onAddToCart }) => {
  ...
  return (
    ...
    >
      {/* Card Container */}
      <div className="glass glass-hover rounded-2xl p-6 h-full backdrop-blur-xl border border-border-dark transition-all duration-300 overflow-hidden">
        {/* Background Glow */}
        <div
        ...
        <div className="relative z-10 space-y-4">
          ...
          <Link to={`/product/${product.id}`} className="block overflow-hidden rounded-xl group/image cursor-pointer">
            <div className="relative w-full h-48 bg-gradient-to-br from-bg-tertiary to-bg-primary flex items-center justify-center overflow-hidden">
              {/* Placeholder for product image */}
              <div
                className={`text-8xl transition-transform duration-300 ${isHovered ? 'scale-110 rotate-3' : ''}`}
                style={{
          ...
          <div className="space-y-3">
            ...
            <Link to={`/product/${product.id}`} className="block">
              <h3 className="text-lg font-bold text-text-primary group-hover:text-neon-cyan transition-colors line-clamp-2">
                {product.name}
              </h3>
            </Link>
          ...
          <div className="space-y-3 pt-4 border-t border-border-dark">
            ...
            <button
              ...
              className="w-full group/btn relative overflow-hidden rounded-lg py-3 font-bold transition-all duration-300"
            >
              {/* Button Background */}
              ...
              <div className="relative flex items-center justify-center gap-2 text-bg-primary group-hover/btn:text-bg-primary transition-colors">
                ...
                <span>{isHovered ? 'Add to Cart' : 'Quick Buy'}</span>
              </div>
            </button>
