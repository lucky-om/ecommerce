...
async function main() {
  ...
  for (const p of PRODUCTS) {
    const imagesJson = JSON.stringify(p.images || [p.imageUrl]).replace(/'/g, "''");
    ...
    sql += `VALUES (${p.id}, '${name}', '${brand}', '${tagline}', '${desc}', ${p.price}, ${p.originalPrice || 'null'}, ${p.categoryId}, '${p.image}', '${imagesJson}'::jsonb, '${specsJson}'::jsonb, ${p.stock}, ${p.rating}, ${p.reviewCount}, ${p.isFeatured}, ${badgeStr}) `;
